...
export class MusickitAPIService {
  ...
  async getLibraryPlaylists(): Promise<MusicKit.LibraryPlaylists[]> {
  ...
  async getLibraryPlaylistSongs(id: number): Promise<MusicKit.LibrarySongs[]> {
    var results = await MusicKit.getInstance().api.v3.music(
      '/v1/me/library/playlists/' +
        id +
        '/tracks?art[url]=f&extend[tracks]=name&extend[catalog]=id&fields[curators]=name,url&fields[songs]=name,artistName,curatorName,composerName,artwork,playParams,contentRating,albumName,url,durationInMillis,audioTraits,extendedAssetUrls&format[resources]=flat&include=tracks,curator,catalog&include[songs]=artists&l=en-US&limit[tracks]=50'
    );
    return results.data.data as MusicKit.LibrarySongs[];
  }
}
